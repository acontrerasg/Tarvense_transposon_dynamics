...
# # Modified by Adrian Contreras Garrido.
# #
...
# # The BAMrc output files ($pop.$pop-insertion.[0/100up/100down].rc) are stored for each $pop in $workspace_dir/$pop/$pop/BAMrc/.
...
# # $project_dir/$pop-insertions.FINAL.NC.DP$depth.bed contains all the NC information but is not filtered for the ratio of negative coverage (ratioNCfilt): for each putative insertion sites it shows for each sample the coverage supporting the non-reference insertion / the coverage supporting the reference absence / the coverage 100bp up / the coverage100bp down.
# # $project_dir/$pop-insertions.ratioNCFINAL.bool.DP$depth.bed filters the putative insertions based on the ratioNCfilt with a 1 for samples where insertions have passed the filter, a 0 where they have not and the reference coverage is sufficient (>$DPrefmin reads) to be confident that the insertion is indeed absent, and - for the samples where the reference coverage is not sufficient to confirm the absence.
# # $project_dir/$pop-insertions.ratioNCFINAL.DP$depth.bed gives the coverage supporting the ratioNCfilt insertions
# # $project_dir/$pop-insertions.ratioNCFINAL.NConly.DP$depth.bed gives the reference coverage over ratioNCfilt insertion sites
## Questions or comments to pbaduel(ar)bio.ens.psl.eu

my $ind_thresh=$ARGV[0] ;  # Minimun number of individuals to be covered
my $depth = $ARGV[1] ; # number of reads (split+discordant) used to call an insertion in Filter_insertion_splitreader.pl
my $path_to_RC = $ARGV[2]; # path to directory where insertion.0.rc insertion.100up.rc and insertion.100down.rc are located.
my $out_dir = $ARGV[3]; # path to store filtered samples
my $filtfile = $ARGV[4] ; # Path to the first pass filter (positive coverage) output (Tarvense_filtered_DP3.bed in my case. )
...
print STDOUT "finding sample index at header of Tarvense_filtered_DP3.bed  \t";
# FIND index of sample column IN header
my @header = split(/\t/, <IN>) ;
foreach my $individual (@pops_to_analyze){
		my $pop = (split(/-/, basename($individual)))[0] ;
	  print STDOUT "$pop \t";
...
foreach my  $pop (sort keys %ind_pop){
	print STDOUT "$pop\t";
	open IN2, "<${path_to_RC}/$pop-insertion.0.rc" ;
	while(<IN2>){
		chomp $_ ;
		my @line = split(/\t/, $_) ;
...
foreach my  $pop (sort keys %ind_pop){
	print STDOUT "$pop\t";
	open IN3, "<${path_to_RC}/${pop}-insertion.100up.rc" ;
	while(<IN3>){
		chomp $_ ;
		my @line = split(/\t/, $_) ;
		# coverage already processed and min cov output for each insertion site
...
foreach my  $pop (sort keys %ind_pop){
	print STDOUT "$pop\t";
	open IN4, "<${path_to_RC}/${pop}-insertion.100down.rc" ;
	while(<IN4>){
		chomp $_ ;
		my @line = split(/\t/, $_) ;
		# coverage already processed and min cov output for each insertion site
...
open OUT, ">${out_dir}/Tarvense-insertions.FINAL.NC.DP$depth.bed" ;
open OUT3, ">${out_dir}/Tarvense-insertions.ratioNC.FINAL.NConly.DP$depth.bed" ;
open OUT4, ">${out_dir}/Tarvense-insertions.ratioNC.FINAL.DP$depth.bed" ;
open OUT5, ">${out_dir}/Tarvense-insertions.ratioNC.FINAL.bool.DP$depth.bed" ;
