...
my $pop = $ARGV[2]; # name of sample
my $InputDir = $ARGV[3];  #path/to/output
my $OutputDir = $ARGV[4];  #path/to/output
my $region = $ARGV[5]; #0 for insertion sites and 100up or 100down for 100bp up or down respectively

print STDERR "Calling Min DP for ".$pop."\n";

my %TE_cov ;
open IN, "<$InputDir/$subsetname-insertion-sites.$region.bed" ;
...
my %ref_cov ;
open IN2, "<$OutputDir/${pop}-insertion.tmp.$region.rc" ;
while(<IN2>){
...
open OUT, ">$OutputDir/${pop}-insertion.$region.rc" ;
