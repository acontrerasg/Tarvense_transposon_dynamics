...
my $depth = $ARGV[0] ; # number of reads (split+discordant) required to call an insertion on 1st pass
my $TSD_info = $ARGV[1]; # path to TSD info
my $fam = $ARGV[2]; # TE family
my $TSDthresh = $ARGV[3]; # threshold above witch presence variants are rejected based on expected TSD length of TE family
my $noTSDthresh = $ARGV[4]; # threshold above witch presence variants are rejected based when no TSD defined for TE family
my $out_dir = $ARGV[5] ;
my @all_pops = @ARGV[6..$#ARGV]; # list of all the genomes analysed in the cohort
...
print STDERR "Filtering Insertion at DP ".$depth."\n";


print STDERR "number of individuals $#all_pops\n";



my @pops_to_analyze;
foreach my $individual (0..$#all_pops){
    my $ind = (split(/-/, $all_pops[$individual]))[1] ;
		push(@pops_to_analyze, $ind)
}

# # retrieve all intervals where putative presence variant calls overlap for a given TE family
my %intersect_array_sort;
open IN, "<${fam}_intersect.sort.bed" ;
while(<IN>){
	chomp $_ ;
	...
	push(@{$intersect_array_sort{$line[0]}{'indNB'}}, $line[3]);  #This is the TE fam and the problem.
}
close IN ;
...
my %intersect_array;
my %peak_array;
open IN, "<${fam}_intersect.mrg.bed" ;
while(<IN>){
	chomp $_ ;
...
my $bedfile  = "${fam}-${pop}-TE-insertion-sites.bed";
	if (-f $bedfile){ #check if file exists
		...
		while(<IN>){
			...
			print STDOUT "$pop\t$line[0]\t$pos_start\t$pos_end\t$split_left\t$split_right\t${$intersect_array{$line[0]}{'start'}}[$intersect_inf]\t${$intersect_array{$line[0]}{'stop'}}[$intersect_sup]\n";
...
open OUT, ">${out_dir}/${fam}_insertions.filt.DP$depth.bed" ;

print OUT "#chrom \t start \t end \t TE_name \t overlap(NBsplit_start/NBsplit_stop) \t" ;
